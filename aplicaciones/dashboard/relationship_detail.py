...
def _resolve_variables(
    ...
    parsed = [
        _parse_variable_key(relationship, key) for key in relationship.variable_ids
    ]
    ...
    for index, stored_key in enumerate(relationship.variable_ids):
        ...
        sensor = ClientSensor.objects.filter(
            client=relationship.client,
...
def _align_series(
    ...
    for bucket in sorted(buckets):
        ...
        for variable in variables:
            slot = buckets[bucket].get(variable["key"])
            values[variable["key"]] = (
        ...
        rows.append({"measured_at": measured_at.isoformat(), "values": values})
    return rows
...
def _series_statistics(rows: list[dict[str, Any]], key: str) -> dict[str, Any]:
    ...
    if not values:
        return {"current": None, "minimum": None, "maximum": None, "average": None, "points": 0}
    return {
        "current": values[-1],
        "minimum": min(values),
        "maximum": max(values),
...
def _find_temperature_humidity_pair(
    ...
    for variable in variables:
        ...
        if temperature is None and any(term in text for term in ("temperature", "temperatura", "air temp")):
            if not any(term in text for term in ("soil", "suelo", "substrate", "sustrato")):
                temperature = variable
        if humidity is None and any(term in text for term in ("relative humidity", "humedad relativa", "humidity")):
            if not any(term in text for term in ("soil", "suelo", "moisture", "sustrato", "substrate")):
                humidity = variable
...
def _vpd_kpa(temperature_c: float, relative_humidity: float) -> float | None:
    if not (-50.0 <= temperature_c <= 70.0) or not (0.0 <= relative_humidity <= 100.0):
        return None
...
def _alert_evaluation(
    ...
    for row in reversed(rows):
        a_value = row["values"].get(alert.variable_a_key)
        b_value = row["values"].get(alert.variable_b_key)
        a_match = _compare(a_value, alert.operator_a, alert.threshold_a)
        b_match = _compare(b_value, alert.operator_b, alert.threshold_b)
        matches = a_match and b_match if alert.logic == "and" else a_match or b_match
        if consecutive == 0:
            latest_matches = matches
        if not matches:
            break
        consecutive += 1
    sustained_minutes = (consecutive * bucket_seconds) / 60.0
    return {
        "conditions_met_now": latest_matches,
        "sustained_minutes": sustained_minutes,
        "triggered_preview": latest_matches and sustained_minutes >= alert.duration_minutes,
    }
...
def agronomy_relationship_alerts(
    ...
    if not can_access_module(request.user, "sensor_configuration"):
        return JsonResponse({"error": "No tienes permiso para configurar alertas."}, status=403)
    ...
    if not name:
        return JsonResponse({"error": "El nombre de la alerta es obligatorio."}, status=400)
    if variable_a_key not in allowed_keys or variable_b_key not in allowed_keys:
        return JsonResponse({"error": "Selecciona variables reales de esta relación."}, status=400)
    if variable_a_key == variable_b_key:
        return JsonResponse({"error": "La alerta debe comparar dos variables diferentes."}, status=400)

    valid_operators = {value for value, _ in AgronomicRelationshipAlert.Operator.choices}
    ...
    if operator_a not in valid_operators or operator_b not in valid_operators:
        return JsonResponse({"error": "Operador de comparación no válido."}, status=400)
    ...
    try:
        ...
        return JsonResponse({"error": "Umbrales, duración y cooldown deben ser numéricos."}, status=400)
    if not math.isfinite(threshold_a) or not math.isfinite(threshold_b):
        return JsonResponse({"error": "Los umbrales deben ser valores finitos."}, status=400)
    if not 1 <= duration_minutes <= 1440:
        return JsonResponse({"error": "La duración debe estar entre 1 y 1440 minutos."}, status=400)
    if not 1 <= cooldown_minutes <= 10080:
        return JsonResponse({"error": "El cooldown debe estar entre 1 minuto y 7 días."}, status=400)

    logic = str(request.POST.get("logic") or "and")
    if logic not in {value for value, _ in AgronomicRelationshipAlert.Logic.choices}:
        logic = AgronomicRelationshipAlert.Logic.AND
    severity = str(request.POST.get("severity") or "medium")
    if severity not in {value for value, _ in AgronomicRelationshipAlert.Severity.choices}:
    ...
    alert, created = AgronomicRelationshipAlert.objects.update_or_create(
        ...
        defaults={
            ...
            "email_recipients": str(request.POST.get("email_recipients") or "").strip()[:500],
            "whatsapp_enabled": request.POST.get("whatsapp_enabled") == "1",
            "whatsapp_recipients": str(request.POST.get("whatsapp_recipients") or "").strip()[:500],
